...
class ScannerWriter():
    def __init__(self, file):
        with open(file, 'rb') as input:
            self.automatonYalex = pickle.load(input)
        self.encabezado = self.trailer = None

    def getEncabezadoTrailer(self):
        encabezado = self.automatonYalex.afds.get("encabezado", None)
        if encabezado and len(encabezado['tokens']) > 0:
            self.encabezado = encabezado['tokens'][0][1:-1]
        trailer = self.automatonYalex.afds.get("rules", None)
        if trailer and len(trailer['tokens']) == 2:
            self.trailer = trailer['tokens'][1]
...
from shunthingYard import shuntingYard"""
            self.getEncabezadoTrailer()
            
            content += "\n#Encabezado del archivo Yalex\n"
            content += self.encabezado if self.encabezado else "#Sin encabezado"
...
def ejecutarAccion(accion):
    ...
    """
            ...
            content += "\n#Clase Automaton"
            content += AUTOMATON_CONTENT

            content += "\n\n#Clase Scanner"
            content += SCANNER_CONTENT

            content += "\n#Trailer\n"
            content += self.trailer if self.trailer else "#Sin trailer"

            with open("./bin/scanner.py", "w", encoding="utf-8") as file:
                file.write(content)
